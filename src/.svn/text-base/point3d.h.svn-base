...
//	CPoint3D(const CPoint3D& P);
	...
	// OpenGL
	void	ToOGL(bool = false) const {glVertex();}
	void	glVertex() const { glVertex3fv(&X[0]); }
	void	glNormal() const { glNormal3fv(&X[0]); }	// Lorsque l'objet est un vecteur normal.
	void	ToGlfloatv(GLfloat* p) const { p[0] = (GLfloat)X[0]; p[1] = (GLfloat)X[1]; p[2] = (GLfloat)X[2]; }
